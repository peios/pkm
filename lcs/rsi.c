...
extern int lcs_rust_write_rsi_query_values_request_frame(
	...
	u8 query_all, const struct pkm_lcs_runtime_limits *limits,
	struct pkm_lcs_rsi_built_request *built);
	...
	struct pkm_lcs_rsi_built_request *built)
{
	...
	return lcs_rust_write_rsi_lookup_request_frame(
		...
		(const u8 *)child_name, child_name_len, limits, built);
}
	...
	struct pkm_lcs_rsi_built_request *built)
{
	...
	return lcs_rust_write_rsi_set_value_request_frame(
		...
		data_len, sequence, expected_sequence, limits, built);
}
	...
	struct pkm_lcs_value_layer_admission_result *result)
{
	...
	return lcs_rust_plan_set_value_layer_admission(
		...
		(const u8 *)value_name, value_name_len,
		(const u8 *)layer_name, layer_name_len, limits, result);
}
	...
	struct pkm_lcs_rsi_built_request *built)
{
	...
	memset(built, 0, sizeof(*built));
	if (!dst || !parent_guid || !child_name || !layer_name || !limits)
		return -EINVAL;

	return lcs_rust_write_rsi_hide_entry_request_frame(
		dst, dst_len, request_id, txn_id, parent_guid,
		(const u8 *)child_name, child_name_len,
		(const u8 *)layer_name, layer_name_len, sequence, limits,
		built);
}
	...
	struct pkm_lcs_rsi_value_watch_events_result *result)
{
	...
	return lcs_rust_materialize_rsi_query_values_watch_events(
		...
		private_layer_count, limits, output, output_len, result);
}
	...
	struct pkm_lcs_rsi_lookup_guid_entry_result *result)
{
	...
	return lcs_rust_materialize_rsi_lookup_guid_entry(
		frame, frame_len, request_id, next_sequence,
		(const u8 *)child_name, child_name_len, target_guid, limits,
		result);
}

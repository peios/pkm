        ...
        if attr.name.eq_ignore_ascii_case(name) {
            ...
            match &attr.values {
                ClaimValues::Int64(vals) => {
                    ...
                    if vals.len() == 1 {
                        return Value { vtype: ValueType::Int64(vals[0]), origin: Origin::UserAttr, flags };
                    }
                    let elements = vals.iter().map(|v| Value {
                        vtype: ValueType::Int64(*v), origin: Origin::UserAttr, flags,
                    }).collect();
                    return Value { vtype: ValueType::Composite(elements), origin: Origin::UserAttr, flags };
                }
                ClaimValues::Uint64(vals) => {
                    if vals.is_empty() { return Value::null(); }
                    if vals.len() == 1 {
                        return Value { vtype: ValueType::Uint64(vals[0]), origin: Origin::UserAttr, flags };
                    }
                    let elements = vals.iter().map(|v| Value {
                ...
                ClaimValues::String(vals) => {
                    if vals.is_empty() { return Value::null(); }
                    if vals.len() == 1 {
                        return Value { vtype: ValueType::String(vals[0].clone()), origin: Origin::UserAttr, flags };
                    }
                    let elements = vals.iter().map(|v| Value {
                        vtype: ValueType::String(v.clone()), origin: Origin::UserAttr, flags,
                    }).collect();
                    return Value { vtype: ValueType::Composite(elements), origin: Origin::UserAttr, flags };
                }
                ClaimValues::Boolean(vals) => {
                    if vals.is_empty() { return Value::null(); }
                    if vals.len() == 1 {
                        return Value { vtype: ValueType::Boolean(vals[0]), origin: Origin::UserAttr, flags };
                    }
                    let elements = vals.iter().map(|v| Value {
                ...
                ClaimValues::Sid(vals) => {
                    if vals.is_empty() { return Value::null(); }
                    if vals.len() == 1 {
                        return Value { vtype: ValueType::Sid(vals[0].clone()), origin: Origin::UserAttr, flags };
                    }
                    let elements = vals.iter().map(|v| Value {
                        vtype: ValueType::Sid(v.clone()), origin: Origin::UserAttr, flags,
                    }).collect();
                    return Value { vtype: ValueType::Composite(elements), origin: Origin::UserAttr, flags };
                }
                ClaimValues::Octet(vals) => {
                    if vals.is_empty() { return Value::null(); }
                    if vals.len() == 1 {
                        return Value { vtype: ValueType::Octet(vals[0].clone()), origin: Origin::UserAttr, flags };
                    }
                    let elements = vals.iter().map(|v| Value {
                        vtype: ValueType::Octet(v.clone()), origin: Origin::UserAttr, flags,
                    }).collect();
                    return Value { vtype: ValueType::Composite(elements), origin: Origin::UserAttr, flags };
                }
            }
    ...
    match (&lhs.vtype, &rhs.vtype) {
        ...
        (ValueType::String(a), ValueType::String(b)) => {
            if cs {
                ...
                Some(a.to_ascii_lowercase().cmp(&b.to_ascii_lowercase()) as i8)
            }
        }
        (ValueType::Octet(a), ValueType::Octet(b)) =>
            Some(a.cmp(b) as i8),
...
fn to_value_set(v: &Value) -> Vec<&Value> {
    match &v.vtype {
        ValueType::Composite(elems) => elems.iter().collect(),
        _ => alloc::vec![v],
    }
}

/// Contains: TRUE if LHS includes ALL of RHS values.
fn set_contains(lhs: &Value, rhs: &Value) -> TriValue {
    if lhs.is_null() || rhs.is_null() {
        return TriValue::Unknown;
    }
    ...
    for r in &rhs_set {
        ...
        if !found {
            if saw_unknown { return TriValue::Unknown; }
            return TriValue::False;
        }
    }
    TriValue::True
...
fn set_any_of(lhs: &Value, rhs: &Value) -> TriValue {
    if lhs.is_null() || rhs.is_null() {
        return TriValue::Unknown;
    }
    let lhs_set = to_value_set(lhs);
    ...
    if saw_unknown { TriValue::Unknown } else { TriValue::False }
}
...
fn to_sid_list(v: &Value) -> Option<Vec<Sid>> {
    match &v.vtype {
        ValueType::Sid(s) => Some(alloc::vec![s.clone()]),
        ValueType::Composite(elems) => {
            if elems.is_empty() { return None; }
            let mut result = Vec::with_capacity(elems.len());
            for e in elems {
                match &e.vtype {
                    ValueType::Sid(s) => result.push(s.clone()),
                    _ => return None,
                }
            }
            Some(result)
...
pub fn evaluate(
    ...
    while pos < condition.len() {
        ...
        match op {
            ...
            0x01 | 0x02 | 0x03 | 0x04 => {
                if pos + 10 > condition.len() { return TriValue::Unknown; }
            ...
            0x10 => {
                if pos + 4 > condition.len() { return TriValue::Unknown; }
                let length = u32::from_le_bytes([
                    condition[pos], condition[pos+1], condition[pos+2], condition[pos+3],
                ]) as usize;
                pos += 4;
                if length > condition.len() - pos { return TriValue::Unknown; }
            ...
            0x18 => {
                ...
                pos += 4;
                if length > condition.len() - pos { return TriValue::Unknown; }
                let data = condition[pos..pos+length].to_vec();
                pos += length;
                stack.push(Value {
            ...
            0x50 => {
                ...
                pos += length;
                match elements {
                    Some(elems) => stack.push(Value {
                        vtype: ValueType::Composite(elems),
                        origin: Origin::Literal,
                        flags: 0,
                    }),
                    None => return TriValue::Unknown,
                }
            }
            ...
            // ==
            0x80 => {
                if stack.len() < 2 { return TriValue::Unknown; }
                let rhs = stack.pop().unwrap();
                let lhs = stack.pop().unwrap();
                stack.push(Value::from_bool_result(compare_equal(&lhs, &rhs)));
            }
            ...
            0x81 => {
                ...
                let rhs = stack.pop().unwrap();
                let lhs = stack.pop().unwrap();
                stack.push(Value::from_bool_result(compare_equal(&lhs, &rhs).negate()));
            }
            ...
            0x82 => {
                ...
                stack.push(Value::from_bool_result(result));
            }
            ...
            0x86 => {
                ...
                let rhs = stack.pop().unwrap();
                let lhs = stack.pop().unwrap();
                stack.push(Value::from_bool_result(set_contains(&lhs, &rhs)));
            }
            ...
            0x88 => {
                ...
                let rhs = stack.pop().unwrap();
                let lhs = stack.pop().unwrap();
                stack.push(Value::from_bool_result(set_any_of(&lhs, &rhs)));
            }
            ...
            0x8e => {
                ...
                let rhs = stack.pop().unwrap();
                let lhs = stack.pop().unwrap();
                stack.push(Value::from_bool_result(set_contains(&lhs, &rhs).negate()));
            }
            ...
            0x8f => {
                ...
                let rhs = stack.pop().unwrap();
                let lhs = stack.pop().unwrap();
                stack.push(Value::from_bool_result(set_any_of(&lhs, &rhs).negate()));
            }
            ...
            // Exists
            0x87 => {
                if stack.len() < 1 { return TriValue::Unknown; }
            ...
            0x89 => {
                ...
                let sids = match to_sid_list(&operand) {
                    Some(s) => s,
                    None => { stack.push(Value::from_bool_result(TriValue::Unknown)); continue; }
                };
            ...
            0x8a => {
                ...
                if token.device_groups.is_none() {
                    stack.push(Value::from_bool_result(TriValue::Unknown));
                    continue;
                }
...
fn parse_composite_elements(data: &[u8]) -> Option<Vec<Value>> {
    ...
    while pos < data.len() {
        ...
        match op {
            ...
            0x10 => {
                ...
                pos += 4;
                if length > data.len() - pos { return None; }
            ...
            0x51 => {
                ...
                pos += 4;
                if length > data.len() - pos { return None; }
                let sid = Sid::from_bytes(&data[pos..pos+length])?;
                pos += length;
                elements.push(Value {
                    vtype: ValueType::Sid(sid),
                    origin: Origin::Literal,
...
mod tests {
    ...
    fn member_of_matching_group() {
        ...
        let admins_sid = well_known::administrators();
    ...
    fn member_of_non_matching() {
        let token = empty_token();
        let random_sid = crate::sid::Sid::new(5, &[21, 999, 999, 999, 9999]);
    ...
    fn member_of_any_one_matches() {
        ...
        let admins = well_known::administrators();
        let random = crate::sid::Sid::new(5, &[21, 1, 2, 3, 999]);

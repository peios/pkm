...
mod tests {
    ...
    fn system_token() -> Token {
        crate::token::Token::system_token()
    }
    ...
    fn audit_success_fires_on_success() {
        let sd = sd_with_sacl_aces(alloc::vec![
            audit_ace(&well_known::everyone(), FILE_READ_DATA, true, false),
        ]);
    ...
    fn audit_failure_fires_on_failure() {
        let sd = sd_with_sacl_aces(alloc::vec![
            audit_ace(&well_known::everyone(), FILE_READ_DATA, false, true),
        ]);
        let token = system_token();
        let result = evaluate_sacl(
            &sd, &token, &FILE_GENERIC_MAPPING,
    ...
    fn audit_both_flags_fires_always() {
        let sd = sd_with_sacl_aces(alloc::vec![
            audit_ace(&well_known::everyone(), FILE_READ_DATA, true, true),
        ]);
        let token = system_token();
    ...
    fn audit_no_flags_never_fires() {
        let sd = sd_with_sacl_aces(alloc::vec![
            audit_ace(&well_known::everyone(), FILE_READ_DATA, false, false),
        ]);
        let token = system_token();
        let result = evaluate_sacl(
            &sd, &token, &FILE_GENERIC_MAPPING,
    ...
    fn audit_sid_must_match() {
        let random_sid = Sid::new(5, &[21, 999, 999, 999, 9999]);
        let sd = sd_with_sacl_aces(alloc::vec![
    ...
    fn audit_mask_must_overlap_granted() {
        ...
        let sd = sd_with_sacl_aces(alloc::vec![
            audit_ace(&well_known::everyone(), FILE_WRITE_DATA, true, false),
        ]);
        let token = system_token();
        let result = evaluate_sacl(
            &sd, &token, &FILE_GENERIC_MAPPING,
    ...
    fn audit_failure_mask_overlaps_denied() {
        ...
        let sd = sd_with_sacl_aces(alloc::vec![
            audit_ace(&well_known::everyone(), FILE_WRITE_DATA, false, true),
        ]);
        let token = system_token();
        let result = evaluate_sacl(
            &sd, &token, &FILE_GENERIC_MAPPING,
    ...
    fn audit_multiple_aces_multiple_events() {
        let sd = sd_with_sacl_aces(alloc::vec![
            audit_ace(&well_known::everyone(), FILE_READ_DATA, true, false),
            audit_ace(&well_known::system(), FILE_READ_DATA, true, false),
        ]);
        let token = system_token();
        let result = evaluate_sacl(
            &sd, &token, &FILE_GENERIC_MAPPING,
    ...
    fn audit_generic_bits_mapped() {
        ...
        let sd = sd_with_sacl_aces(alloc::vec![
            audit_ace(&well_known::everyone(), GENERIC_READ, true, false),
        ]);
        let token = system_token();
        let result = evaluate_sacl(
            &sd, &token, &FILE_GENERIC_MAPPING,
    ...
    fn alarm_sets_continuous_mask() {
        let sd = sd_with_sacl_aces(alloc::vec![
            alarm_ace(&well_known::everyone(), FILE_READ_DATA | FILE_WRITE_DATA),
        ]);
        let token = system_token();
        // Granted read+write
        let result = evaluate_sacl(
    ...
    fn alarm_only_on_success() {
        let sd = sd_with_sacl_aces(alloc::vec![
            alarm_ace(&well_known::everyone(), FILE_READ_DATA),
        ]);
        let token = system_token();
        let result = evaluate_sacl(
            &sd, &token, &FILE_GENERIC_MAPPING,
    ...
    fn alarm_multiple_accumulate() {
        let sd = sd_with_sacl_aces(alloc::vec![
            alarm_ace(&well_known::everyone(), FILE_READ_DATA),
            alarm_ace(&well_known::system(), FILE_WRITE_DATA),
        ]);
        let token = system_token();
        let result = evaluate_sacl(
            &sd, &token, &FILE_GENERIC_MAPPING,
    ...
    fn mixed_audit_and_alarm() {
        let sd = sd_with_sacl_aces(alloc::vec![
            audit_ace(&well_known::everyone(), FILE_READ_DATA, true, false),
            alarm_ace(&well_known::everyone(), FILE_WRITE_DATA),
        ]);
        let token = system_token();
        let result = evaluate_sacl(
            &sd, &token, &FILE_GENERIC_MAPPING,

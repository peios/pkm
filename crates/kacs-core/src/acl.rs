...
impl Acl {
    ...
    pub fn from_bytes(data: &[u8]) -> Option<Self> {
        ...
        for _ in 0..ace_count {
            if offset >= ace_data.len() {
                return None; // claimed more ACEs than data contains
            }
            let (ace, consumed) = Ace::from_bytes(&ace_data[offset..])?;
            offset += consumed;
            aces.push(ace);
        }

        Some(Acl { revision, aces })
    }
    ...
    pub fn to_bytes(&self) -> Vec<u8> {
        ...
        let mut buf = Vec::with_capacity(acl_size as usize);
        buf.push(self.revision);
        buf.push(0); // padding
        buf.extend_from_slice(&acl_size.to_le_bytes());
        buf.extend_from_slice(&ace_count.to_le_bytes());
        buf.extend_from_slice(&[0, 0]); // padding
        buf.extend_from_slice(&ace_bytes);
        buf
    }

    /// Byte length of the serialized ACL.
    pub fn byte_len(&self) -> usize {
        let ace_len: usize = self.aces.iter().map(|a| a.to_bytes().len()).sum();
        ACL_HEADER_SIZE + ace_len
    }
}
...
mod tests {
    ...
    fn empty_acl_round_trip() {
        ...
        let parsed = Acl::from_bytes(&bytes).unwrap();
    ...
    fn single_ace_round_trip() {
        let acl = Acl {
            ...
            aces: alloc::vec![allow_ace(&well_known::administrators(), GENERIC_ALL)],
        };
        let bytes = acl.to_bytes();
        let parsed = Acl::from_bytes(&bytes).unwrap();
        ...
        assert_eq!(parsed.aces[0].sid, well_known::administrators());
    }
    ...
    fn multiple_aces_round_trip() {
        let acl = Acl {
            revision: ACL_REVISION,
            aces: alloc::vec![
                deny_ace(&well_known::guests(), GENERIC_ALL),
                allow_ace(&well_known::administrators(), GENERIC_ALL),
                allow_ace(&well_known::authenticated_users(), FILE_READ_DATA | READ_CONTROL),
                allow_ace(&well_known::everyone(), FILE_READ_DATA),
            ],
        };
        let bytes = acl.to_bytes();
        let parsed = Acl::from_bytes(&bytes).unwrap();
        ...
        assert_eq!(parsed.aces[0].sid, well_known::guests());
        assert_eq!(parsed.aces[1].ace_type, ACCESS_ALLOWED_ACE_TYPE);
        assert_eq!(parsed.aces[1].sid, well_known::administrators());
        assert_eq!(parsed.aces[2].sid, well_known::authenticated_users());
        assert_eq!(parsed.aces[3].sid, well_known::everyone());
    }
    ...
    fn byte_len_matches_serialized() {
        ...
        assert_eq!(acl.byte_len(), acl.to_bytes().len());
    }

    #[test]
    fn reject_truncated_header() {
        assert!(Acl::from_bytes(&[0x02, 0x00, 0x08]).is_none());
    }

    #[test]
    fn reject_invalid_revision() {
        let mut bytes = Acl::new(ACL_REVISION).to_bytes();
        bytes[0] = 0x03; // invalid revision
        assert!(Acl::from_bytes(&bytes).is_none());
    }
    ...
    fn large_acl_many_aces() {
        ...
        for i in 0..50u32 {
            aces.push(allow_ace(
                &crate::sid::Sid::new(5, &[21, 1000000, 2000000, 1000 + i]),
                FILE_READ_DATA,
            ));
        }
        let acl = Acl {

...
pub enum ClaimValues {
    Int64(Vec<i64>),
    Uint64(Vec<u64>),
    String(Vec<alloc::string::String>),
    Sid(Vec<Sid>),
    Boolean(Vec<bool>),
    Octet(Vec<Vec<u8>>),
}
...
mod tests {
    ...
    fn system_token_basics() {
        ...
        assert_eq!(token.user_sid, crate::well_known::system());
    ...
    fn system_token_groups() {
        ...
        assert_eq!(token.groups[0].sid, crate::well_known::administrators());

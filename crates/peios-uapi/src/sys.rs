// Direct kernel syscall invocation. no_std, no libc.
//
...
//
// Callers in `peios-uapi::raw` (and `libp-*::raw`) wrap these in typed
// signatures; the safe layer above invokes them through `eintr_retry`.
...
compile_error!(
    "peios-uapi::sys currently supports only x86_64. \
     Add per-arch asm stubs to crates/peios-uapi/src/sys.rs when porting."
);

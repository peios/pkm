// msgpack — minimal MessagePack parser and encoder. no_std + alloc.
//
// A general codec, deliberately standalone and not Peios-specific: libp-rs
// and the validation tools consume it, but it carries no KACS/KMES coupling
// (the libp-go side uses an off-the-shelf msgpack library in the same role).
//
...
/// Errors produced while decoding msgpack.
...
impl fmt::Display for Error {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Error::Truncated => f.write_str("msgpack input truncated"),
            Error::UnknownType(b) => write!(f, "msgpack unknown type byte 0x{b:02x}"),
            Error::InvalidUtf8 => f.write_str("msgpack string is not valid UTF-8"),
        }
    }
}
...
pub fn parse(buf: &[u8]) -> Result<(Value, &[u8]), Error> {
...
impl<'a> Parser<'a> {
    ...
    fn read_str(&mut self, n: usize) -> Result<Value, Error> {
    ...
    fn read_array(&mut self, n: usize) -> Result<Value, Error> {
    ...
    fn read_map(&mut self, n: usize) -> Result<Value, Error> {
        let mut items = Vec::with_capacity(n);
        for _ in 0..n {
            let k = self.value()?;
            let v = self.value()?;
    ...
    fn read_fixext(&mut self, n: usize) -> Result<Value, Error> {
        let ty = self.read_u8()? as i8;
...
mod tests {
    ...
    fn rejects_unknown_tag() {
        ...
        assert_eq!(err, Error::UnknownType(0xc1));
    }
}

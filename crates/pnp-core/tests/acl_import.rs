...
fn descending_priority_reproduces_first_match_semantics() {
    ...
    let acl = [
        AclLine {
            matches: |s| matches!(s.src_addr, Some(a) if match a {
    ...
    let imported = || {
        vec![
            rb("line-1").s("SrcAddr.Equal", "10.0.0.0/8").int("Priority", 30).actions(&["PASS"]),
            rb("line-2").int("DstPort.Equal", 22).int("Priority", 20).actions(&["DROP"]),
            rb("line-3").s("Protocol.Equal", "tcp").int("Priority", 10).actions(&["PASS"]),
        ]
    };

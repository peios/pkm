use lcs_core::{
    ...
    ValueWriteInput, plan_blanket_tombstone, plan_blanket_tombstone_transaction_log_entry,
    plan_value_delete, plan_value_delete_transaction_log_entry, plan_value_write,
...
fn value_write_log_entry_preserves_value_layer_sequence_and_commit_effects() {
    ...
    let entry = plan_value_write_transaction_log_entry(&limits, &planned, &mut operation_counter)
...
fn delete_and_blanket_log_entries_capture_layer_and_sequence_shape() {
    ...
    .expect("delete plan");
    let delete_entry =
        plan_value_delete_transaction_log_entry(&limits, &delete, &mut operation_counter)
    ...
    let set_entry =
        plan_blanket_tombstone_transaction_log_entry(&limits, &blanket_set, &mut operation_counter)
...
fn corrupt_value_side_effect_flags_fail_before_operation_index_allocation() {
    ...
    assert_eq!(
        plan_blanket_tombstone_transaction_log_entry(&limits, &blanket, &mut operation_counter),
        Err(LcsError::InvalidTransactionMutationLogEntry {
...
fn invalid_value_log_metadata_reuses_value_validation_before_allocation() {
    ...
    assert_eq!(
        plan_value_write_transaction_log_entry(&limits, &planned, &mut operation_counter),

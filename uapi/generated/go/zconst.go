// SPDX-License-Identifier: GPL-2.0-only WITH Linux-syscall-note
// Code generated by gen.sh; DO NOT EDIT.

package uapi

const (
	ACCESS_SYSTEM_SECURITY                              = 16777216
	DACL_SECURITY_INFORMATION                           = 4
	DELETE                                              = 65536
	GENERIC_ALL                                         = 268435456
	GENERIC_EXECUTE                                     = 536870912
	GENERIC_READ                                        = 2147483648
	GENERIC_WRITE                                       = 1073741824
	GROUP_SECURITY_INFORMATION                          = 2
	KACS_ACCESS_ACCESS_SYSTEM_SECURITY                  = 16777216
	KACS_ACCESS_CHECK_ARGS_SIZE                         = 136
	KACS_ACCESS_CHECK_ARGS_V1_SIZE                      = 40
	KACS_ACCESS_CHECK_MAX_AUDIT_CONTEXT_LEN             = 4096
	KACS_ACCESS_CHECK_MAX_LOCAL_CLAIMS_LEN              = 65536
	KACS_ACCESS_CHECK_MAX_OBJECT_TYPE_COUNT             = 1024
	KACS_ACCESS_DELETE                                  = 65536
	KACS_ACCESS_GENERIC_ALL                             = 268435456
	KACS_ACCESS_GENERIC_EXECUTE                         = 536870912
	KACS_ACCESS_GENERIC_READ                            = 2147483648
	KACS_ACCESS_GENERIC_WRITE                           = 1073741824
	KACS_ACCESS_MAXIMUM_ALLOWED                         = 33554432
	KACS_ACCESS_READ_CONTROL                            = 131072
	KACS_ACCESS_SYNCHRONIZE                             = 1048576
	KACS_ACCESS_WRITE_DAC                               = 262144
	KACS_ACCESS_WRITE_OWNER                             = 524288
	KACS_ACE_FLAG_CONTAINER_INHERIT                     = 2
	KACS_ACE_FLAG_FAILED_ACCESS                         = 128
	KACS_ACE_FLAG_INHERITED                             = 16
	KACS_ACE_FLAG_INHERIT_ONLY                          = 8
	KACS_ACE_FLAG_NO_PROPAGATE_INHERIT                  = 4
	KACS_ACE_FLAG_OBJECT_INHERIT                        = 1
	KACS_ACE_FLAG_SUCCESSFUL_ACCESS                     = 64
	KACS_ACE_INHERITED_OBJECT_TYPE_PRESENT              = 2
	KACS_ACE_OBJECT_TYPE_PRESENT                        = 1
	KACS_ACE_TYPE_ACCESS_ALLOWED                        = 0
	KACS_ACE_TYPE_ACCESS_ALLOWED_CALLBACK               = 9
	KACS_ACE_TYPE_ACCESS_ALLOWED_CALLBACK_OBJECT        = 11
	KACS_ACE_TYPE_ACCESS_ALLOWED_COMPOUND               = 4
	KACS_ACE_TYPE_ACCESS_ALLOWED_OBJECT                 = 5
	KACS_ACE_TYPE_ACCESS_DENIED                         = 1
	KACS_ACE_TYPE_ACCESS_DENIED_CALLBACK                = 10
	KACS_ACE_TYPE_ACCESS_DENIED_CALLBACK_OBJECT         = 12
	KACS_ACE_TYPE_ACCESS_DENIED_OBJECT                  = 6
	KACS_ACE_TYPE_SYSTEM_ACCESS_FILTER                  = 21
	KACS_ACE_TYPE_SYSTEM_ALARM                          = 3
	KACS_ACE_TYPE_SYSTEM_ALARM_CALLBACK                 = 14
	KACS_ACE_TYPE_SYSTEM_ALARM_CALLBACK_OBJECT          = 16
	KACS_ACE_TYPE_SYSTEM_ALARM_OBJECT                   = 8
	KACS_ACE_TYPE_SYSTEM_AUDIT                          = 2
	KACS_ACE_TYPE_SYSTEM_AUDIT_CALLBACK                 = 13
	KACS_ACE_TYPE_SYSTEM_AUDIT_CALLBACK_OBJECT          = 15
	KACS_ACE_TYPE_SYSTEM_AUDIT_OBJECT                   = 7
	KACS_ACE_TYPE_SYSTEM_MANDATORY_LABEL                = 17
	KACS_ACE_TYPE_SYSTEM_PROCESS_TRUST_LABEL            = 20
	KACS_ACE_TYPE_SYSTEM_RESOURCE_ATTRIBUTE             = 18
	KACS_ACE_TYPE_SYSTEM_SCOPED_POLICY_ID               = 19
	KACS_ACK_CAAP_LOCK_FAIL                             = 3
	KACS_ACK_EVAL_CONTEXT                               = 1
	KACS_ACK_OK                                         = 0
	KACS_ACK_TOKEN_RESOLVE                              = 2
	KACS_AUDIT_POLICY_OBJECT_ACCESS_FAILURE             = 2
	KACS_AUDIT_POLICY_OBJECT_ACCESS_SUCCESS             = 1
	KACS_AUDIT_POLICY_PRIVILEGE_USE_FAILURE             = 8
	KACS_AUDIT_POLICY_PRIVILEGE_USE_SUCCESS             = 4
	KACS_BACKUP_INTENT                                  = 1
	KACS_CAAP_DESTROY                                   = 3
	KACS_CAAP_INIT                                      = 2
	KACS_CAAP_MAX_ACL_BYTES                             = 65535
	KACS_CAAP_MAX_FIELD_BYTES                           = 65536
	KACS_CAAP_MAX_RULE_COUNT                            = 256
	KACS_CAAP_MAX_SPEC_BYTES                            = 262144
	KACS_CAAP_SET                                       = 1
	KACS_CAAP_SPEC_OFF_RULE_COUNT                       = 1
	KACS_CAAP_SPEC_OFF_VERSION                          = 0
	KACS_CAAP_SPEC_PREFIX_BYTES                         = 5
	KACS_CAAP_SPEC_VERSION                              = 1
	KACS_CAAP_TCB_GATE                                  = 0
	KACS_CAP_ALLOW_GRANT                                = 0
	KACS_CAP_CAPABLE                                    = 6
	KACS_CAP_CAPGET                                     = 7
	KACS_CAP_CAPSET                                     = 4
	KACS_CAP_HARD_DENY                                  = 1
	KACS_CAP_PRCTL_GUARD                                = 5
	KACS_CAP_PRIV_NOT_ENABLED                           = 2
	KACS_CAP_USE_MARK_FAIL                              = 3
	KACS_CLAIM_ATTR_CASE_SENSITIVE                      = 2
	KACS_CLAIM_ATTR_DISABLED                            = 16
	KACS_CLAIM_ATTR_USE_FOR_DENY_ONLY                   = 4
	KACS_CLAIM_TYPE_BOOLEAN                             = 6
	KACS_CLAIM_TYPE_INT64                               = 1
	KACS_CLAIM_TYPE_OCTET                               = 16
	KACS_CLAIM_TYPE_SID                                 = 5
	KACS_CLAIM_TYPE_STRING                              = 3
	KACS_CLAIM_TYPE_UINT64                              = 2
	KACS_CREATE_OPT_DELETE_ON_CLOSE                     = 2
	KACS_CREATE_OPT_DIRECTORY                           = 1
	KACS_CRED_ALLOC_BLANK                               = 2
	KACS_CRED_CLONE_FORK_COPY                           = 6
	KACS_CRED_CLONE_THREAD_SHARE                        = 5
	KACS_CRED_FREE                                      = 3
	KACS_CRED_INSTALL_TOKEN_REF                         = 4
	KACS_CRED_PREPARE                                   = 0
	KACS_CRED_PROJECT_E2BIG                             = 9
	KACS_CRED_PROJECT_GROUPS_ALLOC_FAIL                 = 8
	KACS_CRED_PROJECT_UID0_BLOCKED                      = 7
	KACS_CRED_TRANSFER                                  = 1
	KACS_DISPOSITION_CREATE                             = 2
	KACS_DISPOSITION_OPEN                               = 1
	KACS_DISPOSITION_OPEN_IF                            = 3
	KACS_DISPOSITION_OVERWRITE                          = 4
	KACS_DISPOSITION_OVERWRITE_IF                       = 5
	KACS_DISPOSITION_SUPERSEDE                          = 0
	KACS_ELEVATION_DEFAULT                              = 1
	KACS_ELEVATION_FULL                                 = 2
	KACS_ELEVATION_LIMITED                              = 3
	KACS_EXEC_BAD_ARGS                                  = 1
	KACS_EXEC_CREDS_ALLOW                               = 0
	KACS_EXEC_ID_CHANGE_NO_TOKEN                        = 2
	KACS_EXEC_ID_CHANGE_PRIV_UNSUPPORTED                = 3
	KACS_EXEC_IMPERSONATION_REVERT_FAIL                 = 13
	KACS_EXEC_INTEGRITY_INVALID_SD                      = 12
	KACS_EXEC_INTEGRITY_NO_CACHE                        = 11
	KACS_EXEC_INTEGRITY_NO_ISEC                         = 10
	KACS_EXEC_NPM_DERIVE_FAIL                           = 7
	KACS_EXEC_NPM_NO_FILE                               = 6
	KACS_EXEC_PIP_CAPPED_UNSAFE                         = 17
	KACS_EXEC_PIP_COMMITTED                             = 14
	KACS_EXEC_SIGNATURE_UNVERIFIABLE                    = 16
	KACS_EXEC_TOKEN_CLONE                               = 5
	KACS_EXEC_TOKEN_CLONE_FAIL                          = 9
	KACS_EXEC_TOKEN_INSTALL_FAIL                        = 8
	KACS_EXEC_TOKEN_NPM_DERIVED                         = 4
	KACS_EXEC_UMH_NOT_TCB                               = 15
	KACS_FILE_ADD_FILE                                  = 2
	KACS_FILE_ADD_SUBDIRECTORY                          = 4
	KACS_FILE_APPEND_DATA                               = 4
	KACS_FILE_DELETE_CHILD                              = 64
	KACS_FILE_EXECUTE                                   = 32
	KACS_FILE_LIST_DIRECTORY                            = 1
	KACS_FILE_READ_ATTRIBUTES                           = 128
	KACS_FILE_READ_DATA                                 = 1
	KACS_FILE_READ_EA                                   = 8
	KACS_FILE_TRAVERSE                                  = 32
	KACS_FILE_WRITE_ATTRIBUTES                          = 256
	KACS_FILE_WRITE_DATA                                = 2
	KACS_FILE_WRITE_EA                                  = 16
	KACS_FSOP_ACCESS                                    = 0
	KACS_FSOP_FALLOCATE                                 = 6
	KACS_FSOP_FCNTL                                     = 4
	KACS_FSOP_IOCTL                                     = 2
	KACS_FSOP_LOCK                                      = 3
	KACS_FSOP_MMAP                                      = 7
	KACS_FSOP_MPROTECT                                  = 8
	KACS_FSOP_PERMISSION                                = 1
	KACS_FSOP_SYSFS_WRITE_GATE                          = 10
	KACS_FSOP_TRUNCATE                                  = 5
	KACS_FSOP_WRITE_INTENT                              = 9
	KACS_FSR_APPEND_DENY                                = 3
	KACS_FSR_AUDIT_EMIT_FAIL                            = 5
	KACS_FSR_DECISION                                   = 0
	KACS_FSR_GRANT_DENY                                 = 2
	KACS_FSR_SIGNED_EXEC                                = 1
	KACS_FSR_UNMANAGED_SYSFS                            = 4
	KACS_FW_ALLOWED                                     = 0
	KACS_FW_BELOW_TCB                                   = 3
	KACS_FW_NO_KEY_MATCH                                = 2
	KACS_FW_PROBE_FAILED                                = 5
	KACS_FW_UNSIGNED                                    = 1
	KACS_FW_UNVERIFIABLE                                = 4
	KACS_IMLEVEL_ANONYMOUS                              = 0
	KACS_IMLEVEL_DELEGATION                             = 3
	KACS_IMLEVEL_IDENTIFICATION                         = 1
	KACS_IMLEVEL_IMPERSONATION                          = 2
	KACS_IOC_ADJUST_DEFAULT                             = 1074809609
	KACS_IOC_ADJUST_GROUPS                              = 1083198215
	KACS_IOC_ADJUST_INTERACTIVITY_SCOPE                 = 1074023178
	KACS_IOC_ADJUST_PRIVS                               = 1075333889
	KACS_IOC_DUPLICATE                                  = 3222293250
	KACS_IOC_GET_LINKED_TOKEN                           = 3221506822
	KACS_IOC_IMPERSONATE                                = 19208
	KACS_IOC_INSTALL                                    = 19203
	KACS_IOC_LINK_TOKENS                                = 1074809605
	KACS_IOC_MAGIC                                      = 75
	KACS_IOC_QUERY                                      = 3222293248
	KACS_IOC_RESTRICT                                   = 3223866116
	KACS_IPC_ALLOC                                      = 0
	KACS_IPC_ALL_ACCESS                                 = 983055
	KACS_IPC_CTL                                        = 2
	KACS_IPC_NO_SD                                      = 5
	KACS_IPC_PERMISSION                                 = 1
	KACS_IPC_QUERY_INFORMATION                          = 4
	KACS_IPC_READ                                       = 1
	KACS_IPC_SD_QUERY                                   = 3
	KACS_IPC_SD_SET                                     = 4
	KACS_IPC_SET_INFORMATION                            = 8
	KACS_IPC_WRITE                                      = 2
	KACS_LOGON_SESSION_SPEC_MAX_BYTES                   = 4096
	KACS_LOGON_SESSION_SPEC_MIN_BYTES                   = 15
	KACS_LOGON_SESSION_SPEC_OFF_AUTH_PKG                = 3
	KACS_LOGON_SESSION_SPEC_OFF_AUTH_PKG_LEN            = 1
	KACS_LOGON_SESSION_SPEC_OFF_LOGON_TYPE              = 0
	KACS_LOGON_TYPE_BATCH                               = 4
	KACS_LOGON_TYPE_INTERACTIVE                         = 2
	KACS_LOGON_TYPE_NETWORK                             = 3
	KACS_LOGON_TYPE_NETWORK_CLEARTEXT                   = 8
	KACS_LOGON_TYPE_NEW_CREDENTIALS                     = 9
	KACS_LOGON_TYPE_REMOTE_INTERACTIVE                  = 10
	KACS_LOGON_TYPE_SERVICE                             = 5
	KACS_META_ACL                                       = 5
	KACS_META_BAD_ARGS                                  = 7
	KACS_META_BEGIN_BUSY                                = 2
	KACS_META_CANONICAL_SD                              = 3
	KACS_META_CAPS_XATTR                                = 4
	KACS_META_CONSUME_HIT                               = 1
	KACS_META_DECISION                                  = 0
	KACS_META_GETSECURITY                               = 9
	KACS_META_INTERNAL_SD                               = 8
	KACS_META_SIGNED_EXEC                               = 6
	KACS_MIT_ALL                                        = 1023
	KACS_MIT_CFI                                        = 8
	KACS_MIT_CFIB                                       = 128
	KACS_MIT_CFIF                                       = 64
	KACS_MIT_LSV                                        = 4
	KACS_MIT_NO_CHILD                                   = 32
	KACS_MIT_PIE                                        = 256
	KACS_MIT_SML                                        = 512
	KACS_MIT_TLP                                        = 2
	KACS_MIT_UI_ACCESS                                  = 16
	KACS_MIT_WXP                                        = 1
	KACS_MOUNT_POLICY_ARGS_MIN_SIZE                     = 16
	KACS_MOUNT_POLICY_DENY_MISSING                      = 2
	KACS_MOUNT_POLICY_SYNTHESIZE_EPHEMERAL              = 3
	KACS_MOUNT_POLICY_SYNTHESIZE_PERSISTENT             = 4
	KACS_MOUNT_POLICY_UNMANAGED                         = 1
	KACS_MP_BAD_ARGS                                    = 1
	KACS_MP_FIXED_POLICY                                = 9
	KACS_MP_GET_NO_SECURITY                             = 8
	KACS_MP_GET_OK                                      = 7
	KACS_MP_NO_SECURITY                                 = 2
	KACS_MP_SET_OK                                      = 0
	KACS_MP_TCB_DENIED                                  = 6
	KACS_MP_TEMPLATE_INVALID                            = 5
	KACS_MP_UNMANAGED                                   = 3
	KACS_MP_VALIDATE                                    = 4
	KACS_NOX_BUILD_CREATED_SD                           = 7
	KACS_NOX_DELETE_ON_CLOSE_ARM                        = 8
	KACS_NOX_PREPARE_BAD_ACCESS                         = 4
	KACS_NOX_PREPARE_BAD_DISPOSITION                    = 3
	KACS_NOX_PREPARE_BAD_FLAGS                          = 1
	KACS_NOX_PREPARE_BAD_SD_ARGS                        = 2
	KACS_NOX_PREPARE_OK                                 = 0
	KACS_NOX_PREPARE_UNSUPPORTED                        = 5
	KACS_NOX_RESOLVE                                    = 6
	KACS_NS_DELETE_EXISTING                             = 4
	KACS_NS_DEST                                        = 3
	KACS_NS_PARENT_FALLBACK                             = 1
	KACS_NS_PRIMARY                                     = 0
	KACS_NS_SOURCE                                      = 2
	KACS_OBJECT_TYPE_ENTRY_SIZE                         = 20
	KACS_OBJ_DELETE_ON_CLOSE_UNLINK                     = 0
	KACS_OBJ_SIGNED_EXEC_MUTATION_BLOCKED               = 2
	KACS_OBJ_SIGNED_EXEC_PIN                            = 1
	KACS_OPEN_HOW_MIN_SIZE                              = 16
	KACS_PA_ALLOW                                       = 0
	KACS_PA_BAD_ARGS                                    = 1
	KACS_PA_DEBUG_DENIED                                = 7
	KACS_PA_DEBUG_RESCUE                                = 6
	KACS_PA_NO_SD                                       = 3
	KACS_PA_NO_TARGET                                   = 2
	KACS_PA_PIP_DENIED                                  = 5
	KACS_PA_PIP_DOMINANCE                               = 8
	KACS_PA_SD_ERROR                                    = 4
	KACS_PORT_ALL_ACCESS                                = 131073
	KACS_PORT_BIND                                      = 1
	KACS_PORT_PROTO_ALL                                 = 3
	KACS_PORT_PROTO_TCP                                 = 1
	KACS_PORT_PROTO_UDP                                 = 2
	KACS_PORT_SELECTOR_MAX_LEN                          = 19
	KACS_PRIM_APPLY_COMMIT                              = 3
	KACS_PRIM_IMPERSONATE_INSTALL                       = 4
	KACS_PRIM_IMPERSONATE_REVERT                        = 5
	KACS_PRIM_INSTALL_OK                                = 0
	KACS_PRIM_SD_ALLOC_FAIL                             = 2
	KACS_PRIM_SD_REALLOC                                = 1
	KACS_PRIM_SIBLING_FAILED                            = 7
	KACS_PRIM_SIBLING_REQUEUE                           = 6
	KACS_PRIVILEGE_ATTR_ENABLED                         = 2
	KACS_PRIVILEGE_ATTR_REMOVED                         = 4
	KACS_PRIVILEGE_RESET_ALL_DEFAULTS                   = 2147483648
	KACS_PRIV_CHANGE_NOTIFY                             = 3
	KACS_PRIV_NOT_ENABLED                               = 1
	KACS_PRIV_NULL_OR_ZERO                              = 0
	KACS_PRIV_RCU_ENOMEM_FALLBACK                       = 4
	KACS_PRIV_USE_MARK_FAIL                             = 2
	KACS_PROCESS_DUP_HANDLE                             = 64
	KACS_PROCESS_QUERY_INFORMATION                      = 1024
	KACS_PROCESS_QUERY_LIMITED                          = 4096
	KACS_PROCESS_SET_INFORMATION                        = 512
	KACS_PROCESS_SIGNAL                                 = 2
	KACS_PROCESS_SUSPEND_RESUME                         = 2048
	KACS_PROCESS_TERMINATE                              = 1
	KACS_PROCESS_VM_READ                                = 16
	KACS_PROCESS_VM_WRITE                               = 32
	KACS_PSB_APPLY_CFIB                                 = 5
	KACS_PSB_APPLY_CFIF                                 = 3
	KACS_PSB_APPLY_MM_ACQUIRE                           = 2
	KACS_PSB_APPLY_NORMALIZE                            = 1
	KACS_PSB_APPLY_OK                                   = 0
	KACS_PSB_APPLY_SML                                  = 4
	KACS_PSB_LSV_PIP_DOMINANCE                          = 11
	KACS_PSB_LSV_PROBE                                  = 9
	KACS_PSB_LSV_VERIFY                                 = 10
	KACS_PSB_PIE_ET_EXEC                                = 12
	KACS_PSB_PRCTL_CFIB                                 = 14
	KACS_PSB_PRCTL_PIP                                  = 15
	KACS_PSB_PRCTL_SML                                  = 13
	KACS_PSB_WXP_EXISTING_VMA                           = 8
	KACS_PSB_WXP_MMAP                                   = 6
	KACS_PSB_WXP_MPROTECT                               = 7
	KACS_PST_ALLOC                                      = 0
	KACS_PST_ALLOC_FAIL                                 = 1
	KACS_PST_CLONE_BLOCKED_NOCHILD                      = 8
	KACS_PST_DUMPABLE                                   = 7
	KACS_PST_EXEC_PIP_COMMIT                            = 6
	KACS_PST_EXEC_PIP_STAGE                             = 5
	KACS_PST_FREE                                       = 2
	KACS_PST_INHERIT_FORK                               = 4
	KACS_PST_INHERIT_SHARE                              = 3
	KACS_PST_SD_ALLOC                                   = 9
	KACS_PST_SD_ALLOC_FAIL                              = 10
	KACS_PST_SD_REPLACE                                 = 12
	KACS_PST_SD_WRAP_FAIL                               = 11
	KACS_PST_SOCKET_SD_ALLOC                            = 13
	KACS_PTO_ACCESS_DENIED                              = 4
	KACS_PTO_BAD_ACCESS                                 = 3
	KACS_PTO_BAD_ARGS                                   = 1
	KACS_PTO_CROSS                                      = 6
	KACS_PTO_NO_TARGET                                  = 2
	KACS_PTO_OPEN_OK                                    = 0
	KACS_PTO_SELF                                       = 5
	KACS_RESTORE_INTENT                                 = 2
	KACS_SCM_TOKEN                                      = 1
	KACS_SDC_CORRUPT_EMPTY_OR_OVERSIZE                  = 4
	KACS_SDC_CORRUPT_VALIDATE_FAIL                      = 5
	KACS_SDC_HIT                                        = 0
	KACS_SDC_MISS_NEEDS_SYNTH                           = 3
	KACS_SDC_MISS_NONE                                  = 1
	KACS_SDC_MISS_STALE_GEN                             = 2
	KACS_SDS_ACCESS_DENIED                              = 4
	KACS_SDS_BAD_ARGS                                   = 2
	KACS_SDS_KIND_ACCESS_CHECK                          = 4
	KACS_SDS_KIND_FILE                                  = 1
	KACS_SDS_KIND_PATH                                  = 3
	KACS_SDS_KIND_PROCESS                               = 2
	KACS_SDS_KIND_TOKEN                                 = 0
	KACS_SDS_NO_SD                                      = 5
	KACS_SDS_QUERY_FAIL                                 = 7
	KACS_SDS_QUERY_OK                                   = 0
	KACS_SDS_RESTORE_BYPASS                             = 6
	KACS_SDS_SET_OK                                     = 1
	KACS_SDS_UNMANAGED                                  = 3
	KACS_SD_AT_SYSV_MASK                                = 117440512
	KACS_SD_AT_SYSV_MSG                                 = 33554432
	KACS_SD_AT_SYSV_SEM                                 = 67108864
	KACS_SD_AT_SYSV_SHM                                 = 16777216
	KACS_SD_DACL_AUTO_INHERITED                         = 1024
	KACS_SD_DACL_AUTO_INHERIT_REQ                       = 256
	KACS_SD_DACL_DEFAULTED                              = 8
	KACS_SD_DACL_PRESENT                                = 4
	KACS_SD_DACL_PROTECTED                              = 4096
	KACS_SD_DACL_TRUSTED                                = 64
	KACS_SD_GROUP_DEFAULTED                             = 2
	KACS_SD_HEADER_BYTES                                = 20
	KACS_SD_OWNER_DEFAULTED                             = 1
	KACS_SD_RM_CONTROL_VALID                            = 16384
	KACS_SD_SACL_AUTO_INHERITED                         = 2048
	KACS_SD_SACL_AUTO_INHERIT_REQ                       = 512
	KACS_SD_SACL_DEFAULTED                              = 32
	KACS_SD_SACL_PRESENT                                = 16
	KACS_SD_SACL_PROTECTED                              = 8192
	KACS_SD_SELF_RELATIVE                               = 32768
	KACS_SD_SERVER_SECURITY                             = 128
	KACS_SECINFO_DACL                                   = 4
	KACS_SECINFO_GROUP                                  = 2
	KACS_SECINFO_LABEL                                  = 16
	KACS_SECINFO_OWNER                                  = 1
	KACS_SECINFO_SACL                                   = 8
	KACS_SES_CREATE                                     = 0
	KACS_SES_CREATE_PRIV_DENIED                         = 1
	KACS_SES_CREATE_TOKEN                               = 4
	KACS_SES_CREATE_TOKEN_PRIV_DENIED                   = 5
	KACS_SES_DESTROY                                    = 2
	KACS_SES_DESTROY_PRIV_DENIED                        = 3
	KACS_SETID_SETGID_NO_TOKEN                          = 2
	KACS_SETID_SETGID_PRIV_GATE                         = 3
	KACS_SETID_SETGROUPS_NO_TOKEN                       = 4
	KACS_SETID_SETGROUPS_PRIV_GATE                      = 5
	KACS_SETID_SETUID_NO_TOKEN                          = 0
	KACS_SETID_SETUID_PRIV_GATE                         = 1
	KACS_SE_ASSIGN_PRIMARY_TOKEN_PRIVILEGE              = 8
	KACS_SE_AUDIT_PRIVILEGE                             = 2097152
	KACS_SE_BACKUP_PRIVILEGE                            = 131072
	KACS_SE_CHANGE_NOTIFY_PRIVILEGE                     = 8388608
	KACS_SE_CREATE_SYMBOLIC_LINK_PRIVILEGE              = 34359738368
	KACS_SE_CREATE_TOKEN_PRIVILEGE                      = 4
	KACS_SE_DEBUG_PRIVILEGE                             = 1048576
	KACS_SE_IMPERSONATE_PRIVILEGE                       = 536870912
	KACS_SE_INCREASE_BASE_PRIORITY_PRIVILEGE            = 16384
	KACS_SE_INCREASE_QUOTA_PRIVILEGE                    = 32
	KACS_SE_LOAD_DRIVER_PRIVILEGE                       = 1024
	KACS_SE_LOCK_MEMORY_PRIVILEGE                       = 16
	KACS_SE_MANAGE_VOLUME_PRIVILEGE                     = 268435456
	KACS_SE_PROFILE_SINGLE_PROCESS_PRIVILEGE            = 8192
	KACS_SE_RELABEL_PRIVILEGE                           = 4294967296
	KACS_SE_REMOTE_SHUTDOWN_PRIVILEGE                   = 16777216
	KACS_SE_RESTORE_PRIVILEGE                           = 262144
	KACS_SE_SECURITY_PRIVILEGE                          = 256
	KACS_SE_SHUTDOWN_PRIVILEGE                          = 524288
	KACS_SE_SYSTEMTIME_PRIVILEGE                        = 4096
	KACS_SE_SYSTEM_PROFILE_PRIVILEGE                    = 2048
	KACS_SE_TAKE_OWNERSHIP_PRIVILEGE                    = 512
	KACS_SE_TCB_PRIVILEGE                               = 128
	KACS_SFS_INIT                                       = 4
	KACS_SFS_LOGON_SESSIONS_ACCESS_CHECK                = 2
	KACS_SFS_LOGON_SESSIONS_NO_TOKEN                    = 0
	KACS_SFS_LOGON_SESSIONS_PIP_CONTEXT                 = 1
	KACS_SFS_OPEN_SELF                                  = 3
	KACS_SID_GROUP_ENABLED                              = 4
	KACS_SID_GROUP_ENABLED_BY_DEFAULT                   = 2
	KACS_SID_GROUP_INTEGRITY                            = 32
	KACS_SID_GROUP_INTEGRITY_ENABLED                    = 64
	KACS_SID_GROUP_LOGON_ID                             = 3221225472
	KACS_SID_GROUP_MANDATORY                            = 1
	KACS_SID_GROUP_OWNER                                = 8
	KACS_SID_GROUP_RESOURCE                             = 536870912
	KACS_SID_GROUP_USE_FOR_DENY_ONLY                    = 16
	KACS_SID_MAX_SUB_AUTHORITIES                        = 15
	KACS_SIG_BAD_KEY_TABLE                              = 1
	KACS_SIG_CRYPTO_MISMATCH                            = 5
	KACS_SIG_CRYPTO_UNAVAILABLE                         = 4
	KACS_SIG_ELF_BAD_BLOB                               = 18
	KACS_SIG_ELF_BAD_IDENT                              = 10
	KACS_SIG_ELF_BAD_SHTABLE                            = 11
	KACS_SIG_ELF_BAD_SIG_SECTION                        = 17
	KACS_SIG_ELF_EHDR_READ                              = 9
	KACS_SIG_ELF_HASH_FAIL                              = 19
	KACS_SIG_ELF_MAGIC_READ                             = 7
	KACS_SIG_ELF_NAME_READ                              = 16
	KACS_SIG_ELF_SHDRS_RANGE                            = 12
	KACS_SIG_ELF_SHDR_READ                              = 15
	KACS_SIG_ELF_SHORT_EHDR                             = 8
	KACS_SIG_ELF_SHSTR_READ                             = 13
	KACS_SIG_ELF_STRTAB_RANGE                           = 14
	KACS_SIG_NO_KEY_MATCH                               = 2
	KACS_SIG_PROBE_FOUND                                = 6
	KACS_SIG_SIZE_CHANGED                               = 22
	KACS_SIG_UNSIGNED                                   = 0
	KACS_SIG_VERIFIED                                   = 3
	KACS_SIG_XATTR_BAD_BLOB                             = 20
	KACS_SIG_XATTR_HASH_FAIL                            = 21
	KACS_SOCK_ALREADY_BOUND                             = 11
	KACS_SOCK_ATTACH                                    = 17
	KACS_SOCK_BAD_ARGS                                  = 0
	KACS_SOCK_BAD_LEVEL                                 = 4
	KACS_SOCK_BIND                                      = 12
	KACS_SOCK_CONNECT                                   = 13
	KACS_SOCK_DELIVER                                   = 20
	KACS_SOCK_GATE                                      = 18
	KACS_SOCK_HAVE_SD                                   = 10
	KACS_SOCK_LEVEL_SET                                 = 14
	KACS_SOCK_LISTEN                                    = 21
	KACS_SOCK_NOT_UNIX                                  = 1
	KACS_SOCK_NO_PEER_TOKEN                             = 6
	KACS_SOCK_NO_SD                                     = 9
	KACS_SOCK_NO_SECURITY                               = 2
	KACS_SOCK_NO_TOKEN                                  = 3
	KACS_SOCK_OPEN_TOKEN                                = 15
	KACS_SOCK_OWNER                                     = 25
	KACS_SOCK_PIP_CONTEXT                               = 7
	KACS_SOCK_PORT_BIND                                 = 23
	KACS_SOCK_PORT_TABLE                                = 24
	KACS_SOCK_REGISTER                                  = 19
	KACS_SOCK_RESTAMP                                   = 22
	KACS_SOCK_SD_DECISION                               = 8
	KACS_SOCK_WRONG_STATE                               = 5
	KACS_SO_IMPERSONATION_LEVEL                         = 2
	KACS_SO_PASS_TOKEN                                  = 3
	KACS_SO_PEER_TOKEN                                  = 1
	KACS_SO_RESTAMP                                     = 4
	KACS_STATUS_CREATED                                 = 2
	KACS_STATUS_OPENED                                  = 1
	KACS_STATUS_OVERWRITTEN                             = 3
	KACS_STATUS_SUPERSEDED                              = 4
	KACS_SYSTEM_MANDATORY_LABEL_NO_EXECUTE_UP           = 4
	KACS_SYSTEM_MANDATORY_LABEL_NO_READ_UP              = 1
	KACS_SYSTEM_MANDATORY_LABEL_NO_WRITE_UP             = 2
	KACS_TASK_ALLOC                                     = 2
	KACS_TASK_ALLOC_INHERIT_ENOMEM                      = 1
	KACS_TASK_ALLOC_NO_CHILD_BLOCKED                    = 0
	KACS_TASK_FREE                                      = 3
	KACS_TLP_CHECK_PATH                                 = 0
	KACS_TLP_REPLACE                                    = 1
	KACS_TOKEN_ADJUST_DEFAULT                           = 128
	KACS_TOKEN_ADJUST_GROUPS                            = 64
	KACS_TOKEN_ADJUST_INTERACTIVITY_SCOPE               = 256
	KACS_TOKEN_ADJUST_PRIVS                             = 32
	KACS_TOKEN_ALL_ACCESS                               = 983551
	KACS_TOKEN_ASSIGN_PRIMARY                           = 1
	KACS_TOKEN_CLASS_APPCONTAINER_SID                   = 15
	KACS_TOKEN_CLASS_CAPABILITIES                       = 16
	KACS_TOKEN_CLASS_DEFAULT_DACL                       = 20
	KACS_TOKEN_CLASS_DEVICE_CLAIMS                      = 23
	KACS_TOKEN_CLASS_DEVICE_GROUPS                      = 14
	KACS_TOKEN_CLASS_ELEVATION_TYPE                     = 13
	KACS_TOKEN_CLASS_GROUPS                             = 2
	KACS_TOKEN_CLASS_IMPERSONATION_LEVEL                = 21
	KACS_TOKEN_CLASS_INTEGRITY_LEVEL                    = 5
	KACS_TOKEN_CLASS_INTERACTIVITY_SCOPE                = 8
	KACS_TOKEN_CLASS_LOGON_SID                          = 19
	KACS_TOKEN_CLASS_LOGON_TYPE                         = 18
	KACS_TOKEN_CLASS_MANDATORY_POLICY                   = 17
	KACS_TOKEN_CLASS_ORIGIN                             = 12
	KACS_TOKEN_CLASS_OWNER                              = 6
	KACS_TOKEN_CLASS_PRIMARY_GROUP                      = 7
	KACS_TOKEN_CLASS_PRIVILEGES                         = 3
	KACS_TOKEN_CLASS_PROJECTED_SUPPLEMENTARY_GIDS       = 24
	KACS_TOKEN_CLASS_RESTRICTED_SIDS                    = 9
	KACS_TOKEN_CLASS_SOURCE                             = 10
	KACS_TOKEN_CLASS_STATISTICS                         = 11
	KACS_TOKEN_CLASS_TYPE                               = 4
	KACS_TOKEN_CLASS_USER                               = 1
	KACS_TOKEN_CLASS_USER_CLAIMS                        = 22
	KACS_TOKEN_DUPLICATE                                = 2
	KACS_TOKEN_GROUP_MASK_WORDS                         = 16
	KACS_TOKEN_IMPERSONATE                              = 4
	KACS_TOKEN_LCS_EXT_HEADER_BYTES                     = 16
	KACS_TOKEN_LCS_EXT_OFF_PRIVATE_LAYER_COUNT          = 12
	KACS_TOKEN_LCS_EXT_OFF_RESERVED                     = 4
	KACS_TOKEN_LCS_EXT_OFF_SCOPE_COUNT                  = 8
	KACS_TOKEN_LCS_EXT_OFF_VERSION                      = 0
	KACS_TOKEN_LCS_EXT_VERSION                          = 1
	KACS_TOKEN_LCS_MAX_LAYER_NAME_BYTES                 = 255
	KACS_TOKEN_LCS_MAX_PRIVATE_LAYERS                   = 256
	KACS_TOKEN_LCS_MAX_SCOPE_GUIDS                      = 256
	KACS_TOKEN_LCS_SCOPE_GUID_BYTES                     = 16
	KACS_TOKEN_MANDATORY_POLICY_NEW_PROCESS_MIN         = 2
	KACS_TOKEN_MANDATORY_POLICY_NO_WRITE_UP             = 1
	KACS_TOKEN_MAX_GROUPS                               = 1024
	KACS_TOKEN_OPEN_REAL                                = 1
	KACS_TOKEN_QUERY                                    = 8
	KACS_TOKEN_QUERY_SOURCE                             = 16
	KACS_TOKEN_RESTRICT_WRITE_RESTRICTED                = 1
	KACS_TOKEN_SPEC_HEADER_BYTES                        = 192
	KACS_TOKEN_SPEC_MAX_BYTES                           = 65536
	KACS_TOKEN_SPEC_MIN_BYTES                           = 192
	KACS_TOKEN_SPEC_OFF_AUDIT_POLICY                    = 44
	KACS_TOKEN_SPEC_OFF_CONFINEMENT_CAPS_COUNT          = 152
	KACS_TOKEN_SPEC_OFF_CONFINEMENT_CAPS_OFFSET         = 148
	KACS_TOKEN_SPEC_OFF_CONFINEMENT_EXEMPT              = 156
	KACS_TOKEN_SPEC_OFF_CONFINEMENT_SID_LEN             = 144
	KACS_TOKEN_SPEC_OFF_CONFINEMENT_SID_OFFSET          = 140
	KACS_TOKEN_SPEC_OFF_DEFAULT_DACL_LEN                = 104
	KACS_TOKEN_SPEC_OFF_DEFAULT_DACL_OFFSET             = 100
	KACS_TOKEN_SPEC_OFF_DEVICE_CLAIMS_LEN               = 120
	KACS_TOKEN_SPEC_OFF_DEVICE_CLAIMS_OFFSET            = 116
	KACS_TOKEN_SPEC_OFF_DEVICE_GROUPS_COUNT             = 128
	KACS_TOKEN_SPEC_OFF_DEVICE_GROUPS_OFFSET            = 124
	KACS_TOKEN_SPEC_OFF_EXPIRATION                      = 48
	KACS_TOKEN_SPEC_OFF_GROUPS_COUNT                    = 96
	KACS_TOKEN_SPEC_OFF_GROUPS_OFFSET                   = 92
	KACS_TOKEN_SPEC_OFF_IMPERSONATION_LEVEL             = 5
	KACS_TOKEN_SPEC_OFF_INTEGRITY_RID                   = 8
	KACS_TOKEN_SPEC_OFF_INTERACTIVITY_SCOPE             = 184
	KACS_TOKEN_SPEC_OFF_ISOLATION_BOUNDARY              = 159
	KACS_TOKEN_SPEC_OFF_LCS_CREDENTIALS_OFFSET          = 188
	KACS_TOKEN_SPEC_OFF_LOGON_SESSION_ID                = 56
	KACS_TOKEN_SPEC_OFF_MANDATORY_POLICY                = 12
	KACS_TOKEN_SPEC_OFF_ORIGIN                          = 176
	KACS_TOKEN_SPEC_OFF_OWNER_SID_INDEX                 = 64
	KACS_TOKEN_SPEC_OFF_PRIMARY_GROUP_INDEX             = 68
	KACS_TOKEN_SPEC_OFF_PRIVS_ENABLED                   = 24
	KACS_TOKEN_SPEC_OFF_PRIVS_PRESENT                   = 16
	KACS_TOKEN_SPEC_OFF_PROJECTED_GID                   = 40
	KACS_TOKEN_SPEC_OFF_PROJECTED_UID                   = 36
	KACS_TOKEN_SPEC_OFF_RESERVED0                       = 6
	KACS_TOKEN_SPEC_OFF_RESERVED1                       = 32
	KACS_TOKEN_SPEC_OFF_RESTRICTED_DEVICE_GROUPS_COUNT  = 172
	KACS_TOKEN_SPEC_OFF_RESTRICTED_DEVICE_GROUPS_OFFSET = 168
	KACS_TOKEN_SPEC_OFF_RESTRICTED_SIDS_COUNT           = 136
	KACS_TOKEN_SPEC_OFF_RESTRICTED_SIDS_OFFSET          = 132
	KACS_TOKEN_SPEC_OFF_SOURCE_ID                       = 80
	KACS_TOKEN_SPEC_OFF_SOURCE_NAME                     = 72
	KACS_TOKEN_SPEC_OFF_SUPP_GIDS_COUNT                 = 164
	KACS_TOKEN_SPEC_OFF_SUPP_GIDS_OFFSET                = 160
	KACS_TOKEN_SPEC_OFF_TOKEN_TYPE                      = 4
	KACS_TOKEN_SPEC_OFF_USER_CLAIMS_LEN                 = 112
	KACS_TOKEN_SPEC_OFF_USER_CLAIMS_OFFSET              = 108
	KACS_TOKEN_SPEC_OFF_USER_DENY_ONLY                  = 158
	KACS_TOKEN_SPEC_OFF_USER_SID_OFFSET                 = 88
	KACS_TOKEN_SPEC_OFF_VERSION                         = 0
	KACS_TOKEN_SPEC_OFF_WRITE_RESTRICTED                = 157
	KACS_TOKEN_SPEC_SOURCE_NAME_BYTES                   = 8
	KACS_TOKEN_SPEC_VERSION                             = 2
	KACS_TOKEN_TYPE_IMPERSONATION                       = 2
	KACS_TOKEN_TYPE_PRIMARY                             = 1
	KACS_TOK_ADJUST_DEFAULT                             = 9
	KACS_TOK_ADJUST_GROUPS                              = 2
	KACS_TOK_ADJUST_INTERACTIVITY_SCOPE                 = 10
	KACS_TOK_ADJUST_PRIVS                               = 1
	KACS_TOK_DUPLICATE                                  = 3
	KACS_TOK_GET_LINKED                                 = 7
	KACS_TOK_IMPERSONATE                                = 8
	KACS_TOK_INSTALL                                    = 4
	KACS_TOK_LINK                                       = 6
	KACS_TOK_QUERY                                      = 0
	KACS_TOK_RESTRICT                                   = 5
	KACS_TOK_UNKNOWN                                    = 11
	KACS_TREF_BIND                                      = 2
	KACS_TREF_OPEN                                      = 3
	KACS_TREF_RELEASE                                   = 1
	KACS_TREF_TO_FD                                     = 0
	KACS_TR_BAD_ARGS                                    = 1
	KACS_TR_CHANGE_NOTIFY_PRIV                          = 13
	KACS_TR_CHANGE_NOTIFY_PRIV_EXHAUSTED                = 14
	KACS_TR_DECISION                                    = 0
	KACS_TR_DELETE_ON_CLOSE_PENDING                     = 7
	KACS_TR_LAZY_DENTRY_RELOOKUP                        = 11
	KACS_TR_NATIVE_ARM                                  = 9
	KACS_TR_NATIVE_STAMP                                = 8
	KACS_TR_NEGATIVE_AFTER_CREATE                       = 12
	KACS_TR_NO_DENTRY_ALIAS                             = 6
	KACS_TR_NO_ISEC                                     = 2
	KACS_TR_NO_TOKEN                                    = 5
	KACS_TR_PIP_CONTEXT                                 = 4
	KACS_TR_STAMP                                       = 10
	KACS_TR_UNMANAGED                                   = 3
	KEY_ALL_ACCESS                                      = 983103
	KEY_CREATE_LINK                                     = 32
	KEY_CREATE_SUB_KEY                                  = 4
	KEY_ENUMERATE_SUB_KEYS                              = 8
	KEY_NOTIFY                                          = 16
	KEY_QUERY_VALUE                                     = 1
	KEY_READ                                            = 131097
	KEY_SET_VALUE                                       = 2
	KEY_WRITE                                           = 131078
	KMES_ATTACH_QUERY_SLOTS                             = 4294967295
	KMES_ATTACH_REQUIRED_PRIVILEGE                      = 256
	KMES_BATCH_MAX_ENTRIES                              = 256
	KMES_CONFIG_BUFFER_CAPACITY_DEFAULT                 = 4194304
	KMES_CONFIG_BUFFER_CAPACITY_MAX                     = 268435456
	KMES_CONFIG_BUFFER_CAPACITY_MIN                     = 65536
	KMES_CONFIG_BUFFER_CAPACITY_TYPE                    = 11
	KMES_CONFIG_KEY_BUFFER_CAPACITY                     = "BufferCapacity"
	KMES_CONFIG_KEY_MAX_EMIT_RATE_PER_PROCESS           = "MaxEmitRatePerProcess"
	KMES_CONFIG_KEY_MAX_EVENT_SIZE                      = "MaxEventSize"
	KMES_CONFIG_KEY_MAX_NESTING_DEPTH                   = "MaxNestingDepth"
	KMES_CONFIG_MAX_EMIT_RATE_PER_PROCESS_DEFAULT       = 10000
	KMES_CONFIG_MAX_EMIT_RATE_PER_PROCESS_MAX           = 1000000
	KMES_CONFIG_MAX_EMIT_RATE_PER_PROCESS_MIN           = 100
	KMES_CONFIG_MAX_EMIT_RATE_PER_PROCESS_TYPE          = 4
	KMES_CONFIG_MAX_EVENT_SIZE_DEFAULT                  = 65536
	KMES_CONFIG_MAX_EVENT_SIZE_MAX                      = 4194304
	KMES_CONFIG_MAX_EVENT_SIZE_MIN                      = 1024
	KMES_CONFIG_MAX_EVENT_SIZE_TYPE                     = 4
	KMES_CONFIG_MAX_NESTING_DEPTH_DEFAULT               = 32
	KMES_CONFIG_MAX_NESTING_DEPTH_MAX                   = 256
	KMES_CONFIG_MAX_NESTING_DEPTH_MIN                   = 4
	KMES_CONFIG_MAX_NESTING_DEPTH_TYPE                  = 4
	KMES_CONFIG_ROOT_HIVE                               = "Machine"
	KMES_CONFIG_ROOT_KMES_KEY                           = "KMES"
	KMES_CONFIG_ROOT_SYSTEM_KEY                         = "System"
	KMES_CONFIG_TYPE_REG_DWORD                          = 4
	KMES_CONFIG_TYPE_REG_QWORD                          = 11
	KMES_CONSUMER_NEED_WAKE_OFFSET                      = 0
	KMES_DROP_BATCH_STRUCT_INVALID                      = 3
	KMES_DROP_RING_FULL                                 = 0
	KMES_DROP_TAIL_RESYNC                               = 1
	KMES_DROP_VALIDATE                                  = 2
	KMES_EMIT_REQUIRED_PRIVILEGE                        = 2097152
	KMES_EVENT_CPU_ID_OFFSET                            = 24
	KMES_EVENT_EFFECTIVE_TOKEN_GUID_OFFSET              = 27
	KMES_EVENT_GUID_SIZE                                = 16
	KMES_EVENT_HEADER_BASE_SIZE                         = 77
	KMES_EVENT_HEADER_SIZE_OFFSET                       = 4
	KMES_EVENT_ORIGIN_CLASS_OFFSET                      = 26
	KMES_EVENT_PROCESS_GUID_OFFSET                      = 59
	KMES_EVENT_SEQUENCE_OFFSET                          = 16
	KMES_EVENT_SIZE_OFFSET                              = 0
	KMES_EVENT_TIMESTAMP_NS_OFFSET                      = 8
	KMES_EVENT_TRUE_TOKEN_GUID_OFFSET                   = 43
	KMES_EVENT_TYPE_LEN_OFFSET                          = 75
	KMES_INGRESS_BATCH_PARTIAL                          = 4
	KMES_INGRESS_EMIT_OVERSIZE                          = 3
	KMES_INGRESS_OVER_CAP_HALF                          = 1
	KMES_INGRESS_OVER_MAX                               = 0
	KMES_INGRESS_SIZE_OVERFLOW                          = 2
	KMES_MAPPING_CONSUMER_OFFSET                        = 4096
	KMES_MAPPING_DATA_OFFSET                            = 8192
	KMES_MAPPING_PRODUCER_OFFSET                        = 0
	KMES_METADATA_PAGE_SIZE                             = 4096
	KMES_METADATA_TOTAL_SIZE                            = 8192
	KMES_ORIGIN_KACS                                    = 2
	KMES_ORIGIN_KMES                                    = 1
	KMES_ORIGIN_LCS                                     = 3
	KMES_ORIGIN_PNP                                     = 4
	KMES_ORIGIN_USERSPACE                               = 0
	KMES_PRODUCER_CAPACITY_OFFSET                       = 16
	KMES_PRODUCER_CPU_ID_OFFSET                         = 12
	KMES_PRODUCER_DATA_OFFSET_OFFSET                    = 24
	KMES_PRODUCER_FUTEX_COUNTER_OFFSET                  = 128
	KMES_PRODUCER_GENERATION_OFFSET                     = 32
	KMES_PRODUCER_MAGIC_OFFSET                          = 0
	KMES_PRODUCER_TAIL_POS_OFFSET                       = 72
	KMES_PRODUCER_VERSION_OFFSET                        = 8
	KMES_PRODUCER_WRITE_POS_OFFSET                      = 64
	KMES_RATE_RECONFIGURE                               = 1
	KMES_RATE_THROTTLE                                  = 0
	KMES_RING_ALLOC                                     = 0
	KMES_RING_CONSUMER_FD                               = 3
	KMES_RING_FREE                                      = 1
	KMES_RING_MAGIC                                     = "KMESRING"
	KMES_RING_PRODUCER_PAGE                             = 2
	KMES_RING_VERSION                                   = 1
	KMES_SWAP_BEGIN                                     = 0
	KMES_SWAP_COMPLETE                                  = 1
	KMES_SWAP_FAILED                                    = 3
	KMES_SWAP_MIGRATE_SKIP                              = 2
	KMES_VAL_EINVAL                                     = 0
	KMES_WAKE_FUTEX                                     = 1
	KMES_WAKE_NOTE                                      = 0
	LCS_AUDIT_BACKUP_COMPLETE                           = 2
	LCS_AUDIT_BACKUP_START                              = 1
	LCS_AUDIT_KEY_OPEN                                  = 0
	LCS_AUDIT_RESTORE_COMPLETE                          = 4
	LCS_AUDIT_RESTORE_START                             = 3
	LCS_AUDIT_SELF_CONFIG_INVALID                       = 6
	LCS_AUDIT_VALIDATION_FAILURE                        = 5
	LCS_BOOT_COMPLETE                                   = 4
	LCS_BOOT_KMES                                       = 1
	LCS_BOOT_LAYERS                                     = 2
	LCS_BOOT_REGISTRY                                   = 0
	LCS_BOOT_SELF_CONFIG_PARAM_INVALID                  = 6
	LCS_BOOT_SELF_CONFIG_REFRESH                        = 5
	LCS_BOOT_SELF_WATCH                                 = 3
	LCS_IF_DELIVERED                                    = 1
	LCS_IF_INSERT                                       = 0
	LCS_IF_RELEASE                                      = 2
	LCS_KCMD_BACKUP                                     = 14
	LCS_KCMD_BLANKET_TOMBSTONE                          = 3
	LCS_KCMD_DELETE_KEY                                 = 4
	LCS_KCMD_DELETE_VALUE                               = 2
	LCS_KCMD_ENUM_SUBKEYS                               = 9
	LCS_KCMD_ENUM_VALUES                                = 8
	LCS_KCMD_FLUSH                                      = 13
	LCS_KCMD_GET_SECURITY                               = 11
	LCS_KCMD_HIDE_KEY                                   = 5
	LCS_KCMD_NONE                                       = 0
	LCS_KCMD_NOTIFY                                     = 16
	LCS_KCMD_QUERY_KEY_INFO                             = 10
	LCS_KCMD_QUERY_VALUE                                = 6
	LCS_KCMD_QUERY_VALUES_BATCH                         = 7
	LCS_KCMD_RESTORE                                    = 15
	LCS_KCMD_SET_SECURITY                               = 12
	LCS_KCMD_SET_VALUE                                  = 1
	LCS_LIM_ALL                                         = 19
	LCS_LIM_MAX_BOUND_TRANSACTIONS_PER_SOURCE           = 9
	LCS_LIM_MAX_CONCURRENT_RSI_REQUESTS                 = 14
	LCS_LIM_MAX_HIVES_PER_SOURCE                        = 13
	LCS_LIM_MAX_KEY_DEPTH                               = 5
	LCS_LIM_MAX_LAYERS_PER_VALUE                        = 8
	LCS_LIM_MAX_PATH_COMPONENT_LENGTH                   = 6
	LCS_LIM_MAX_PRIVATE_LAYERS_PER_TOKEN                = 16
	LCS_LIM_MAX_READ_ONLY_TRANSACTIONS_PER_SOURCE       = 10
	LCS_LIM_MAX_REGISTERED_SOURCES                      = 12
	LCS_LIM_MAX_SCOPE_GUIDS_PER_TOKEN                   = 15
	LCS_LIM_MAX_SUBTREE_WATCH_DEPTH                     = 17
	LCS_LIM_MAX_TOTAL_LAYERS                            = 11
	LCS_LIM_MAX_TOTAL_PATH_LENGTH                       = 7
	LCS_LIM_MAX_TRANSACTION_WATCH_EVENT_BURST           = 18
	LCS_LIM_MAX_VALUE_SIZE                              = 4
	LCS_LIM_NOTIFICATION_QUEUE_SIZE                     = 2
	LCS_LIM_REQUEST_TIMEOUT_MS                          = 0
	LCS_LIM_SYMLINK_DEPTH_LIMIT                         = 3
	LCS_LIM_TRANSACTION_TIMEOUT_MS                      = 1
	LCS_OP_BLANKET_TOMBSTONE                            = 6
	LCS_OP_CREATE_ENTRY                                 = 8
	LCS_OP_CREATE_KEY                                   = 11
	LCS_OP_DELETE_ENTRY                                 = 10
	LCS_OP_DELETE_LAYER                                 = 17
	LCS_OP_DELETE_VALUE                                 = 5
	LCS_OP_DROP_KEY                                     = 7
	LCS_OP_ENUM_CHILDREN                                = 2
	LCS_OP_FLUSH                                        = 16
	LCS_OP_HIDE_ENTRY                                   = 9
	LCS_OP_LOOKUP                                       = 0
	LCS_OP_QUERY_VALUES                                 = 3
	LCS_OP_READ_KEY                                     = 1
	LCS_OP_SET_VALUE                                    = 4
	LCS_OP_TXN_ABORT                                    = 15
	LCS_OP_TXN_BEGIN                                    = 13
	LCS_OP_TXN_COMMIT                                   = 14
	LCS_OP_WRITE_KEY                                    = 12
	LCS_REG_COPY                                        = 2
	LCS_REG_NEW                                         = 0
	LCS_REG_OVERFLOW_FAIL                               = 4
	LCS_REG_REPLAY_FAIL                                 = 3
	LCS_REG_RESUME_DOWN                                 = 1
	LCS_RESP_ACCEPTED                                   = 0
	LCS_RESP_DESYNC                                     = 1
	LCS_RESP_LATE_BEGIN_FAIL                            = 7
	LCS_RESP_LATE_COMMIT_FAIL                           = 5
	LCS_RESP_LATE_MUTATION_FAIL                         = 6
	LCS_RESP_MALFORMED_PAYLOAD                          = 4
	LCS_RESP_OP_MISMATCH                                = 2
	LCS_RESP_UNKNOWN_STATUS                             = 3
	LCS_ROUTE_ABSOLUTE_PATH                             = 1
	LCS_ROUTE_HIVE_NAME                                 = 0
	LCS_ROUTE_SYMLINK_TARGET                            = 2
	LCS_SRC_EXPLICIT                                    = 3
	LCS_SRC_MALFORMED                                   = 2
	LCS_SRC_MARK_BY_ID                                  = 4
	LCS_SRC_OPEN                                        = 0
	LCS_SRC_RELEASE                                     = 1
	LCS_TXN_ST_ABORTED                                  = 3
	LCS_TXN_ST_ACTIVE_BOUND                             = 1
	LCS_TXN_ST_ACTIVE_UNBOUND                           = 0
	LCS_TXN_ST_COMMITTED                                = 2
	LCS_TXN_ST_SOURCE_DOWN                              = 5
	LCS_TXN_ST_TIMED_OUT                                = 4
	MAXIMUM_ALLOWED                                     = 33554432
	OWNER_SECURITY_INFORMATION                          = 1
	PEIOS_PNP_ABI_VERSION                               = 4
	PEIOS_PNP_COMM_LEN                                  = 16
	PEIOS_PNP_COUNTER_MAX_WINDOWS                       = 8
	PEIOS_PNP_COUNTER_NAME_LEN                          = 64
	PEIOS_PNP_EV_ATTR_LEN                               = 96
	PEIOS_PNP_EV_DIR_IN                                 = 0
	PEIOS_PNP_EV_DIR_OUT                                = 1
	PEIOS_PNP_EV_FLOW_ABSENT                            = 0
	PEIOS_PNP_EV_FLOW_ESTABLISHED                       = 2
	PEIOS_PNP_EV_FLOW_INVALID                           = 4
	PEIOS_PNP_EV_FLOW_NEW                               = 1
	PEIOS_PNP_EV_FLOW_RELATED                           = 3
	PEIOS_PNP_EV_FLOW_UNTRACKED                         = 5
	PEIOS_PNP_EV_F_BACKSTOP                             = 1
	PEIOS_PNP_EV_F_FAIL_CLOSED                          = 2
	PEIOS_PNP_EV_F_IDENTITY_UNRESOLVED                  = 16
	PEIOS_PNP_EV_F_REJECT_DEGRADED                      = 4
	PEIOS_PNP_EV_F_REJUDGED                             = 8
	PEIOS_PNP_EV_LAYER_FLOW                             = 2
	PEIOS_PNP_EV_LAYER_PACKET                           = 0
	PEIOS_PNP_EV_LAYER_RAWPACKET                        = 1
	PEIOS_PNP_EV_LOCAL_ABSENT                           = 0
	PEIOS_PNP_EV_LOCAL_KERNEL                           = 2
	PEIOS_PNP_EV_LOCAL_NONE                             = 4
	PEIOS_PNP_EV_LOCAL_PROGRAM                          = 1
	PEIOS_PNP_EV_LOCAL_SHARED                           = 3
	PEIOS_PNP_EV_REJECT_PROHIBITED                      = 1
	PEIOS_PNP_EV_REJECT_REFUSED                         = 0
	PEIOS_PNP_EV_SEAT_EGRESS                            = 2
	PEIOS_PNP_EV_SEAT_INGRESS                           = 1
	PEIOS_PNP_EV_SEAT_LOCAL_IN                          = 3
	PEIOS_PNP_EV_SEAT_LOCAL_OUT                         = 4
	PEIOS_PNP_EV_VERDICT_DROP                           = 2
	PEIOS_PNP_EV_VERDICT_PASS                           = 0
	PEIOS_PNP_EV_VERDICT_REJECT                         = 1
	PEIOS_PNP_FLOW_MAX_TAGS                             = 8
	PEIOS_PNP_FLOW_SENTENCES                            = 2
	PEIOS_PNP_GUID_LEN                                  = 16
	PEIOS_PNP_IOC_COUNTERS                              = 3222818306
	PEIOS_PNP_IOC_COUNTERS_NR                           = 2
	PEIOS_PNP_IOC_FLOWS                                 = 3222818307
	PEIOS_PNP_IOC_FLOWS_NR                              = 3
	PEIOS_PNP_IOC_LISTENERS                             = 3222818308
	PEIOS_PNP_IOC_LISTENERS_NR                          = 4
	PEIOS_PNP_IOC_STATUS                                = 2170572289
	PEIOS_PNP_IOC_STATUS_NR                             = 1
	PEIOS_PNP_IOC_TYPE                                  = 78
	PEIOS_PNP_KEY_DST_ADDR                              = 2
	PEIOS_PNP_KEY_INTERFACE                             = 4
	PEIOS_PNP_KEY_SRC_ADDR                              = 1
	PEIOS_PNP_SERVICE_SID_LEN                           = 32
	PEIOS_PNP_SID_LEN                                   = 68
	READ_CONTROL                                        = 131072
	REG_BACKUP_ARGS_SIZE                                = 4
	REG_BACKUP_BLANKET_TOMBSTONE                        = 6
	REG_BACKUP_HEADER                                   = 1
	REG_BACKUP_KEY                                      = 3
	REG_BACKUP_LAYER                                    = 2
	REG_BACKUP_MAGIC                                    = "PEIOSREG"
	REG_BACKUP_PATH_ENTRY                               = 4
	REG_BACKUP_TRAILER                                  = 255
	REG_BACKUP_VALUE                                    = 5
	REG_BINARY                                          = 3
	REG_BLANKET_TOMBSTONE_ARGS_SIZE                     = 24
	REG_CREATED_NEW                                     = 1
	REG_CREATE_KEY_ARGS_SIZE                            = 48
	REG_DELETE_KEY_ARGS_SIZE                            = 24
	REG_DELETE_VALUE_ARGS_SIZE                          = 40
	REG_DWORD                                           = 4
	REG_DWORD_BIG_ENDIAN                                = 5
	REG_ENUM_SUBKEY_ARGS_SIZE                           = 40
	REG_ENUM_VALUE_ARGS_SIZE                            = 40
	REG_EXPAND_SZ                                       = 2
	REG_FULL_RESOURCE_DESCRIPTOR                        = 9
	REG_GET_SECURITY_ARGS_SIZE                          = 16
	REG_HIDE_KEY_ARGS_SIZE                              = 24
	REG_IOC_BACKUP                                      = 1074024974
	REG_IOC_BACKUP_NR                                   = 14
	REG_IOC_BLANKET_TOMBSTONE                           = 1075335683
	REG_IOC_BLANKET_TOMBSTONE_NR                        = 3
	REG_IOC_COMMIT                                      = 21008
	REG_IOC_COMMIT_NR                                   = 16
	REG_IOC_DELETE_KEY                                  = 1075335688
	REG_IOC_DELETE_KEY_NR                               = 8
	REG_IOC_DELETE_VALUE                                = 1076384258
	REG_IOC_DELETE_VALUE_NR                             = 2
	REG_IOC_ENUM_SUBKEYS                                = 3223867910
	REG_IOC_ENUM_SUBKEYS_NR                             = 6
	REG_IOC_ENUM_VALUES                                 = 3223867909
	REG_IOC_ENUM_VALUES_NR                              = 5
	REG_IOC_FLUSH                                       = 21005
	REG_IOC_FLUSH_NR                                    = 13
	REG_IOC_GET_SECURITY                                = 3222295050
	REG_IOC_GET_SECURITY_NR                             = 10
	REG_IOC_HIDE_KEY                                    = 1075335689
	REG_IOC_HIDE_KEY_NR                                 = 9
	REG_IOC_NOTIFY                                      = 1074287116
	REG_IOC_NOTIFY_NR                                   = 12
	REG_IOC_QUERY_KEY_INFO                              = 3225440775
	REG_IOC_QUERY_KEY_INFO_NR                           = 7
	REG_IOC_QUERY_VALUE                                 = 3225440768
	REG_IOC_QUERY_VALUES_BATCH                          = 3222819332
	REG_IOC_QUERY_VALUES_BATCH_NR                       = 4
	REG_IOC_QUERY_VALUE_NR                              = 0
	REG_IOC_RESTORE                                     = 1074024975
	REG_IOC_RESTORE_NR                                  = 15
	REG_IOC_SET_SECURITY                                = 1075335691
	REG_IOC_SET_SECURITY_NR                             = 11
	REG_IOC_SET_VALUE                                   = 1077957121
	REG_IOC_SET_VALUE_NR                                = 1
	REG_IOC_TXN_STATUS                                  = 2148028945
	REG_IOC_TXN_STATUS_NR                               = 17
	REG_IOC_TYPE                                        = 82
	REG_LINK                                            = 6
	REG_MULTI_SZ                                        = 7
	REG_NONE                                            = 0
	REG_NOTIFY_ALL                                      = 7
	REG_NOTIFY_ARGS_SIZE                                = 8
	REG_NOTIFY_SD                                       = 4
	REG_NOTIFY_SUBKEY                                   = 2
	REG_NOTIFY_VALUE                                    = 1
	REG_OPENED_EXISTING                                 = 2
	REG_OPEN_LINK                                       = 1
	REG_OPTION_CREATE_LINK                              = 2
	REG_OPTION_VOLATILE                                 = 1
	REG_QUERY_KEY_INFO_ARGS_SIZE                        = 64
	REG_QUERY_VALUES_BATCH_ARGS_SIZE                    = 24
	REG_QUERY_VALUE_ARGS_SIZE                           = 64
	REG_QWORD                                           = 11
	REG_RESOURCE_LIST                                   = 8
	REG_RESOURCE_REQUIREMENTS_LIST                      = 10
	REG_RESTORE_ARGS_SIZE                               = 4
	REG_SET_SECURITY_ARGS_SIZE                          = 24
	REG_SET_VALUE_ARGS_SIZE                             = 64
	REG_SRC_HIVE_ENTRY_SIZE                             = 56
	REG_SRC_REGISTER                                    = 1075335680
	REG_SRC_REGISTER_ARGS_SIZE                          = 24
	REG_SRC_REGISTER_NR                                 = 0
	REG_SZ                                              = 1
	REG_TOMBSTONE                                       = 65535
	REG_TXN_ABORTED                                     = 3
	REG_TXN_ACTIVE_BOUND                                = 1
	REG_TXN_ACTIVE_UNBOUND                              = 0
	REG_TXN_COMMITTED                                   = 2
	REG_TXN_SOURCE_DOWN                                 = 5
	REG_TXN_STATUS_ARGS_SIZE                            = 8
	REG_TXN_TIMED_OUT                                   = 4
	REG_VALID_ACE_ACCESS_MASK                           = 4044292159
	REG_VALID_DESIRED_ACCESS_MASK                       = 4077846591
	REG_VALID_MAPPED_ACCESS_MASK                        = 17760319
	REG_VALID_SECURITY_INFORMATION                      = 15
	REG_WATCH_EVENT_MIN_SIZE                            = 8
	REG_WATCH_EVENT_NAME_LEN_OFFSET                     = 6
	REG_WATCH_EVENT_NAME_OFFSET                         = 8
	REG_WATCH_EVENT_TOTAL_LEN_OFFSET                    = 0
	REG_WATCH_EVENT_TYPE_OFFSET                         = 4
	REG_WATCH_KEY_DELETED                               = 6
	REG_WATCH_OVERFLOW                                  = 7
	REG_WATCH_PATH_COMPONENT_LEN_SIZE                   = 2
	REG_WATCH_SD_CHANGED                                = 5
	REG_WATCH_SUBKEY_CREATED                            = 3
	REG_WATCH_SUBKEY_DELETED                            = 4
	REG_WATCH_SUBTREE_PATH_COMPONENTS_REL_OFFSET        = 2
	REG_WATCH_SUBTREE_PATH_DEPTH_REL_OFFSET             = 0
	REG_WATCH_SUBTREE_PATH_DEPTH_SIZE                   = 2
	REG_WATCH_VALUE_DELETED                             = 2
	REG_WATCH_VALUE_SET                                 = 1
	RSI_ABORT_TRANSACTION                               = 50
	RSI_ABORT_TRANSACTION_RESPONSE                      = 32818
	RSI_ALREADY_EXISTS                                  = 2
	RSI_BEGIN_TRANSACTION                               = 48
	RSI_BEGIN_TRANSACTION_RESPONSE                      = 32816
	RSI_CAS_FAILED                                      = 8
	RSI_COMMIT_TRANSACTION                              = 49
	RSI_COMMIT_TRANSACTION_RESPONSE                     = 32817
	RSI_CREATE_ENTRY                                    = 2
	RSI_CREATE_ENTRY_RESPONSE                           = 32770
	RSI_CREATE_KEY                                      = 16
	RSI_CREATE_KEY_RESPONSE                             = 32784
	RSI_DELETE_ENTRY                                    = 4
	RSI_DELETE_ENTRY_RESPONSE                           = 32772
	RSI_DELETE_LAYER                                    = 80
	RSI_DELETE_LAYER_RESPONSE                           = 32848
	RSI_DELETE_VALUE_ENTRY                              = 34
	RSI_DELETE_VALUE_ENTRY_RESPONSE                     = 32802
	RSI_DROP_KEY                                        = 19
	RSI_DROP_KEY_RESPONSE                               = 32787
	RSI_ENUM_CHILDREN                                   = 5
	RSI_ENUM_CHILDREN_RESPONSE                          = 32773
	RSI_FLUSH                                           = 64
	RSI_FLUSH_RESPONSE                                  = 32832
	RSI_GUID_SIZE                                       = 16
	RSI_HIDE_ENTRY                                      = 3
	RSI_HIDE_ENTRY_RESPONSE                             = 32771
	RSI_HIVE_PRIVATE                                    = 1
	RSI_INVALID                                         = 7
	RSI_LENGTH_PREFIX_SIZE                              = 4
	RSI_LOOKUP                                          = 1
	RSI_LOOKUP_RESPONSE                                 = 32769
	RSI_MIN_RESPONSE_SIZE                               = 18
	RSI_NOT_EMPTY                                       = 4
	RSI_NOT_FOUND                                       = 1
	RSI_OK                                              = 0
	RSI_PATH_TARGET_GUID                                = 0
	RSI_PATH_TARGET_HIDDEN                              = 1
	RSI_QUERY_VALUES                                    = 32
	RSI_QUERY_VALUES_RESPONSE                           = 32800
	RSI_READ_KEY                                        = 17
	RSI_READ_KEY_RESPONSE                               = 32785
	RSI_REQUEST_HEADER_SIZE                             = 22
	RSI_REQUEST_ID_OFFSET                               = 4
	RSI_REQUEST_OP_CODE_OFFSET                          = 12
	RSI_REQUEST_TOTAL_LEN_OFFSET                        = 0
	RSI_REQUEST_TXN_ID_OFFSET                           = 14
	RSI_RESPONSE_BIT                                    = 32768
	RSI_RESPONSE_HEADER_SIZE                            = 14
	RSI_RESPONSE_ID_OFFSET                              = 4
	RSI_RESPONSE_OP_CODE_OFFSET                         = 12
	RSI_RESPONSE_STATUS_OFFSET                          = 14
	RSI_RESPONSE_TOTAL_LEN_OFFSET                       = 0
	RSI_SET_BLANKET_TOMBSTONE                           = 35
	RSI_SET_BLANKET_TOMBSTONE_RESPONSE                  = 32803
	RSI_SET_VALUE                                       = 33
	RSI_SET_VALUE_RESPONSE                              = 32801
	RSI_STATUS_SIZE                                     = 4
	RSI_STORAGE_ERROR                                   = 3
	RSI_TOO_LARGE                                       = 5
	RSI_TXN_BUSY                                        = 6
	RSI_TXN_NOT_SUPPORTED                               = 9
	RSI_TXN_READ_ONLY                                   = 1
	RSI_TXN_READ_WRITE                                  = 0
	RSI_WRITE_KEY                                       = 18
	RSI_WRITE_KEY_FIELD_KNOWN_MASK                      = 3
	RSI_WRITE_KEY_FIELD_LAST_WRITE_TIME                 = 2
	RSI_WRITE_KEY_FIELD_SD                              = 1
	RSI_WRITE_KEY_RESPONSE                              = 32786
	SACL_SECURITY_INFORMATION                           = 8
	SOL_KACS                                            = 4096
	SYS_KACS_ACCESS_CHECK                               = 1023
	SYS_KACS_ACCESS_CHECK_LIST                          = 1024
	SYS_KACS_CREATE_LOGON_SESSION                       = 1004
	SYS_KACS_CREATE_TOKEN                               = 1003
	SYS_KACS_DESTROY_EMPTY_LOGON_SESSION                = 1006
	SYS_KACS_GET_MOUNT_POLICY                           = 1026
	SYS_KACS_GET_SD                                     = 1021
	SYS_KACS_OPEN                                       = 1020
	SYS_KACS_OPEN_PROCESS_TOKEN                         = 1001
	SYS_KACS_OPEN_SELF_TOKEN                            = 1000
	SYS_KACS_OPEN_THREAD_TOKEN                          = 1002
	SYS_KACS_REVERT                                     = 1012
	SYS_KACS_SET_CAAP                                   = 1025
	SYS_KACS_SET_MOUNT_POLICY                           = 1027
	SYS_KACS_SET_PSB                                    = 1005
	SYS_KACS_SET_SD                                     = 1022
	SYS_KMES_ATTACH                                     = 1091
	SYS_KMES_EMIT                                       = 1090
	SYS_KMES_EMIT_BATCH                                 = 1092
	SYS_REG_BEGIN_TRANSACTION                           = 1102
	SYS_REG_CREATE_KEY                                  = 1101
	SYS_REG_OPEN_KEY                                    = 1100
	WRITE_DAC                                           = 262144
	WRITE_OWNER                                         = 524288
)
